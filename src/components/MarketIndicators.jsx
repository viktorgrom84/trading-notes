...
import {
  ...
  ThemeIcon, ActionIcon, SimpleGrid, RingProgress, Paper, Button
} from '@mantine/core'
import { LineChart } from '@mantine/charts'
import { IconRefresh, IconAlertTriangle, IconCircleCheck, IconInfoCircle } from '@tabler/icons-react'
          ...
          <>
                ...
                <Group gap="xl" align="center">
                  ...
                  <div>
                    ...
                    <Text c="dimmed" size="sm" mb={4}>
                      As of {current?.date
                        ? new Date(current.date + '-02').toLocaleDateString('en-US', { month: 'long', year: 'numeric' })
                        : '—'}
                    </Text>
                ...
                <Stack gap="xs" align="flex-end">
                  ...
                  <Paper withBorder p="sm" radius="md" style={{ minWidth: 160, textAlign: 'right' }}>
                    ...
                    <Text fw={700} size="lg" c={current?.value > avg ? 'red' : 'green'}>
                      {current
                        ? `${((current.value / avg - 1) * 100).toFixed(0)}% ${current.value > avg ? 'above' : 'below'}`
                        : '—'}
                    </Text>
                  </Paper>
                  <Paper withBorder p="sm" radius="md" style={{ minWidth: 160, textAlign: 'right' }}>
                    ...
                    <Text fw={700} size="lg" c="blue">
                      ~{current ? impliedReturn(current.value) : '—'}% / yr
                    </Text>
                  </Paper>
                </Stack>
              </Group>
            ...
            <Card withBorder p="xl" radius="md">
              ...
              <LineChart
                ...
                referenceLines={[{ y: avg, label: `Avg ~${avg}`, color: 'gray' }]}
              />
            </Card>
            ...
            <SimpleGrid cols={{ base: 2, sm: 3, lg: 6 }}>
              {ZONES.map(z => (
                <Card
                  key={z.label} withBorder p="sm" radius="md"
                  style={{
                    borderColor: zone?.label === z.label ? z.hex : undefined,
                    borderWidth: zone?.label === z.label ? 2 : 1,
                  }}
                >
                  <Group gap="xs" mb={4}>
            ...
            <Card withBorder p="xl" radius="md">
              ...
              <Stack gap="sm">
                <Text size="sm">
                  The <strong>Cyclically Adjusted Price-to-Earnings Ratio</strong>, developed by Nobel laureate Robert Shiller,
                  divides the current S&P 500 price by the{' '}
                  <strong>10-year inflation-adjusted average earnings</strong>. This smooths out short-term earnings swings
                  caused by recessions and booms.
                </Text>
